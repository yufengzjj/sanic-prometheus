...
def init(app, latency_buckets=None, multiprocess_mode='all',
    ...
    app.config.metrics['RQS_LATENCY'] = Histogram(
    ...
    if memcollect_enabled:
        app.config.metrics['PROC_RSS_MEM_BYTES'] = Gauge(
            'sanic_mem_rss_bytes',
            'Resident memory used by process running Sanic',
            multiprocess_mode=multiprocess_mode
        )
        app.config.metrics['PROC_RSS_MEM_PERC'] = Gauge(
            'sanic_mem_rss_perc',
...
def after_request_handler(request, response, get_endpoint_fn):
    ...
    request.app.config.metrics['RQS_LATENCY'].labels(
        request.method, endpoint, response_status
    ).observe(lat)
    request.app.config.metrics['RQS_COUNT'].labels(
        request.method, endpoint, response_status
    ).inc()
